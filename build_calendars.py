...
def build_month_tasks():
    ...
    # The rounds repeat every month, so they travel once rather than twelve
    # times. Titles only, with the "Monthly: " prefix trimmed because the
    # column they land in already says so.
    rounds = [title.replace("Monthly: ", "", 1) for _d, _s, title, _b in MONTHLY_ROUNDS]
    lines = [
        "/* Generated by build_calendars.py. Do not edit: edit TASKS there. */",
        "window.MONTH_TASKS = " + json.dumps(months, indent=2) + ";",
        "window.MONTHLY_ROUNDS = " + json.dumps(rounds) + ";",
    ]
    return "\n".join(lines) + "\n"
