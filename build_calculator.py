...
WIDGET_OUT = os.path.join("docs", "calc-widget.js")

# The home page's inline "how much life is left?" panel, per the approved
# mockups. Generated here so the data and the arithmetic are the same ones
# the calculator page runs: two copies of a lifespan would drift on the
# first correction. The compact view assumes "replaced on schedule", which
# is the calculator page's own default; the full page is one click away for
# anyone who knows better.
WIDGET_TEMPLATE = """/* Generated by build_calculator.py. Do not edit: edit it there. */
(function () {
  var SYSTEMS = @SYSTEMS@;
  var NOW = new Date().getFullYear();
...
def build_widget(items):
    data = json.dumps(
        [{"name": r["name"], "life": r["life"], "range": r["range"]}
         for r in items], ensure_ascii=False).replace("</", "<\\/")
    text = WIDGET_TEMPLATE.replace("@SYSTEMS@", data)
    with open(WIDGET_OUT, "w", encoding="utf-8", newline="\n") as handle:
        handle.write(text)
    print("{0}  the home page's inline panel".format(WIDGET_OUT))


if __name__ == "__main__":
    build_free()
    build_widget(free_rows())
