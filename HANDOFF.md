...
|---|---|---|
| `/` | Say what this is, prove it is Gulf specific, hand over one calendar | Hand-written `docs/index.html` |
| `/calendars/` | The four feeds one at a time, plus all the setup help | Hand-written `docs/calendars/index.html` |
| `/shop/` | Sell the three printables | Hand-written `docs/shop/index.html` |
...
**The 1.25.0 redesign, from an external design review Chad commissioned and
told a session to follow in full.** The review's frame, now the site's:
a Gulf Coast field guide, styled the way the product PDFs already are.
Newsreader and Inter from Google Fonts (stacks in `theme.css` as
`--font-serif`/`--font-sans`, full system fallbacks, fonts origins added to
every measured page's CSP, disclosed in `privacy.html`), the PDF palette in
`theme.css`, one nav bar on every hand-written page (Calendar, Storm prep,
Tools, Resources, Shop, plus the one filled free-calendar button), a
structured footer, and the home page rebuilt in the review's section order,
including a "this month" hero card and section filled from
`docs/month-tasks.js`, which `build_calendars.py` now generates from `TASKS`
(titles only, the free half by construction). The home page's phone-screen
counts in older notes predate this rebuild.

**Three of the review's ideas were adapted on purpose, not skipped by
accident.** Readable interior pages would reverse the 1.10.0 paid-content
...
**Do not spend another session on the site's layout** without a reason from
the analytics or a direct instruction from Chad. The 1.25.0 redesign was the
second kind: Chad commissioned an external design review and told a session to
...
analytics, in the sitemap. Its route for people has moved twice: a footer
link alone was a route for crawlers (Chad caught that within the hour, fixed
in 1.24.1 with a hero line), and 1.25.0 replaced the hero line with two
permanent homes, the Resources item in the nav bar on every page and the
"From the resource shelf" card section on the home page. A second resource
page joins that section as a card; a third means a resources index page. The
